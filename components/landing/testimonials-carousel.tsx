    ...
    >
      <CarouselContent className="-ml-3 md:-ml-5">
        {testimonials.map((testimonial, index) => (
          <CarouselItem key={testimonial.src} className="basis-[86%] pl-3 sm:basis-[62%] md:basis-[44%] md:pl-5 lg:basis-[34%]">
            <figure className="relative overflow-hidden border border-ink/15 bg-snow">
      ...
      <div className="mt-8 flex items-center justify-center gap-5">
        <button
          ...
          className="inline-flex size-11 items-center justify-center border border-ink/25 text-ink transition-colors hover:border-signal hover:bg-snow hover:text-signal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal"
        ...
        <div className="flex items-center gap-2" aria-label={`Depoimento ${current + 1} de ${testimonials.length}`}>
          {testimonials.map((testimonial, index) => (
            <button
              ...
              className={cn(
                "h-1.5 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal",
                current === index ? "w-8 bg-signal" : "w-2 bg-ink/20 hover:bg-ink/45",
              )}
