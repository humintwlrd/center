...
// Posts do Instagram que NÃO devem ser publicados como artigos no site.
// Adicione aqui o shortCode (case-sensitive) do post a ser excluído.
const EXCLUDED_SHORT_CODES = new Set([
  "DRQKUQ6knd8", // Encontre o Governante
  "DQxjbOQEZ2B", // Encontre o Sábio
  "DQptK6GkSin", // Encontre o Herói
  "DMyEFrEx99v", // Quer persuadir ou influenciar alguém?
])
...
function resolveHeroImage(post, category) {
  ...
  if (shortCode) {
    const localFile = path.join(instagramImagesDir, `${shortCode}.jpg`)
    if (fs.existsSync(localFile)) {
      const altSource = cleanText(removeHashtagCloudLines(post.caption ?? "")) || fallback.alt
      return {
        src: `/images/instagram/${shortCode}.jpg`,
        alt: smartTrim(altSource, 140),
      }
    }
  }
  return fallback
...
function main() {
  ...
  console.log(
    `Gerados ${articles.length} artigos em ${path.relative(projectRoot, outputPath)}` +
      (excludedCount > 0 ? ` (${excludedCount} posts excluídos via EXCLUDED_SHORT_CODES)` : ""),
  )
}
