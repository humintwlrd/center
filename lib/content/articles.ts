...
/**
 * Identifica imagens originárias dos posts do Instagram, baixadas pelo script
 * `scripts/download-instagram-images.mjs` e armazenadas em `public/images/instagram/`.
 * Usamos isso para preservar a proporção original da imagem no mobile.
 */
export function isInstagramImage(src?: string | null) {
  return !!src && src.startsWith("/images/instagram/")
}

/**
 * Conta palavras de um Article, atravessando todos os blocos textuais.
 * Usado para preencher `wordCount` no schema.org Article (sinal de profundidade
 * editorial para Google/IA).
 */
export function getArticleWordCount(article: Article): number {
  ...
  let total = 0
  total += countWords(article.title) + countWords(article.description)
  for (const b of article.body) {
    switch (b.type) {
      ...
      case "note":
        ...
        break
      case "quote":
        total += countWords(b.text) + (b.cite ? countWords(b.cite) : 0)
        break
      case "ul":
      case "ol":
        total += b.items.reduce((acc, it) => acc + countWords(it), 0)
        break
    }
  }
  return total
